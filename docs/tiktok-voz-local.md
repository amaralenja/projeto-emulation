...
as bibliotecas. Cada geração libera a GPU ao terminar: prepare as falas antes
de ligar muitos emuladores. Não há garantia de geração em tempo real.

No Windows, a geração é bloqueada com menos de 3,5 GiB de RAM disponíveis ou
...
1. Abra TikTok Lives e escreva uma frase de até 300 caracteres.
...
Nenhum áudio é transmitido automaticamente quando a geração termina.
...
Validação: `python -m unittest test_voice test_voice_http test_panel_upload`, dentro de `camvideo`.
...
a versão atual na primeira porta livre entre 8768 e 8775. Instâncias antigas
