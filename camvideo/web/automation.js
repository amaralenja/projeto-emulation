(() => {
  ...
  function drawCatalog(state){
    ...
    if(hash!==catalogHash){catalogHash=hash;document.getElementById('catalog-choices').innerHTML=[...new Set([...(catalog.tasks||[]),...Object.keys(links)])].map((task,i)=>`<div class="catalog-row" data-task="${esc(task)}"><label><input class="catalog-check" type="checkbox" ${Object.hasOwn(links,task)?'checked':''}>${esc(task)}</label>${(catalog.tasks||[]).includes(task)?'':'<small>Fora do catálogo atual — desmarque ou substitua.</small>'}<select aria-label="Vídeo para ${esc(task)}" class="catalog-video"><option value="">Selecione um vídeo</option>${(state.videos||[]).map(v=>`<option value="${esc(v.name)}" ${links[task]===v.name?'selected':''}>${esc(v.name)}</option>`).join('')}</select></div>`).join('');}
