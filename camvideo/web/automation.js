(() => {
  ...
  render = state => {
    ...
    document.getElementById('automation-queue-status').textContent = state.queueBatch ? `Rodada ${state.queueBatch} • ${state.queueSaved?.length || 0} salvo(s) • Aguardando: ${(state.queuePending || []).map(phoneLabel).join(', ') || 'ninguém'}${state.queueFreeGiB != null ? ' • RAM livre ao iniciar: ' + state.queueFreeGiB.toFixed(1) + ' GiB' : ''}` : '';
